...
function updateHtmlComponent(data) {
	// Trim and encode the financial statements
	...
	if (data.incStatement || data.balSheet || data.cfStatement) {
		wixLocation.to(`/due-diligence-report?income_statement=${encodedIncomeStatement}&balance_sheet=${encodedBalanceSheet}&cf_statement=${encodedCfStatement}&cim_file=${encodedCIMFile}&thesis_prompt=${encodedThesisPrompt}`);
	}
}
